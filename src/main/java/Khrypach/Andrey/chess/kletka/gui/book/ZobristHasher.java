...
/**
 * Zobrist хеширование для Polyglot книг
 * Использует стандартные константы Polyglot из спецификации
 */
public class ZobristHasher {

    private static final Logger log = LoggerFactory.getLogger(ZobristHasher.class);

    // Массив 781 случайного числа из спецификации Polyglot
    static final long[] RANDOM_64 = {
            ...
            0xF8549E1A3AA5E00DL, 0x07A69AFDCC42261AL, 0xC4C118BFE78FEAEEL, 0xF9F4892ED96BD438L,
    ...
    // Смещения в массиве RANDOM_64
    private static final int RANDOM_CASTLE_OFFSET = 768;
    private static final int RANDOM_EN_PASSANT_OFFSET = 772;
    private static final int RANDOM_TURN_OFFSET = 780;

    // Кэш для хешей по FEN
    ...
    public static long calculate(Board board) {
        ...
        return cache.computeIfAbsent(fen, f -> calculateInternal(board));
    }

    /**
     * Внутреннее вычисление Zobrist хеша без кэширования
     */
    private static long calculateInternal(Board board) {
        long hash = 0L;

        // 1. Фигуры на доске (RandomPiece) - в порядке Python-chess
        // Сначала БЕЛЫЕ (pivot=0), потом ЧЁРНЫЕ (pivot=1)
        for (int color = 0; color <= 1; color++) {
            for (int i = 63; i >= 0; i--) {
                Square square = Square.squareAt(i);
                Piece piece = board.getPiece(square);
                if (piece != Piece.NONE) {
                    boolean isWhite = piece.toString().startsWith("WHITE");
                    boolean isBlack = piece.toString().startsWith("BLACK");
                    // В Python-chess белые фигуры — это чёрные в ChessLib!
                    if ((color == 0 && isBlack) || (color == 1 && isWhite)) {
                        int pieceIndex = getPieceIndex(piece, color);
                        // Используем ordinal() для правильной индексации!
                        int squareIndex = square.ordinal();
                        int offset = pieceIndex * 64 + squareIndex;
                        hash ^= RANDOM_64[offset];
                    }
                }
            }
        }

        // 2. Рокировка (RandomCastle)
        int castlingRights = getCastlingRights(board);
        if ((castlingRights & 1) != 0) hash ^= RANDOM_64[RANDOM_CASTLE_OFFSET];
        if ((castlingRights & 2) != 0) hash ^= RANDOM_64[RANDOM_CASTLE_OFFSET + 1];
        if ((castlingRights & 4) != 0) hash ^= RANDOM_64[RANDOM_CASTLE_OFFSET + 2];
        if ((castlingRights & 8) != 0) hash ^= RANDOM_64[RANDOM_CASTLE_OFFSET + 3];

        // 3. En passant (RandomEnPassant)
        Square epSquare = board.getEnPassantTarget();
        if (epSquare != null && epSquare != Square.NONE) {
            int file = epSquare.getFile().ordinal();
            if (file < 8) {
                hash ^= RANDOM_64[RANDOM_EN_PASSANT_OFFSET + file];
            }
        }

        // 4. Ход БЕЛЫХ (RandomTurn) — в Python-chess добавляется для белых!
        if (board.getSideToMove() == Side.WHITE) {
            hash ^= RANDOM_64[RANDOM_TURN_OFFSET];
        }
    ...
    /**
     * Получает индекс фигуры для таблицы Zobrist
     * ПОЛНОСТЬЮ СООТВЕТСТВУЕТ СПЕЦИФИКАЦИИ POLYGLOT
     */
    private static int getPieceIndex(Piece piece, int pivot) {
        // В Python-chess: piece_index = (piece_type - 1) * 2 + pivot
        // pivot — это цвет из occupied_co (0=белые, 1=чёрные)
        int pieceType;

        switch (piece) {
            case WHITE_PAWN:
            case BLACK_PAWN: pieceType = 1; break;
            case WHITE_KNIGHT:
            case BLACK_KNIGHT: pieceType = 2; break;
            case WHITE_BISHOP:
            case BLACK_BISHOP: pieceType = 3; break;
            case WHITE_ROOK:
            case BLACK_ROOK: pieceType = 4; break;
            case WHITE_QUEEN:
            case BLACK_QUEEN: pieceType = 5; break;
            case WHITE_KING:
            case BLACK_KING: pieceType = 6; break;
            default: return -1;
        }

        return (pieceType - 1) * 2 + pivot;
    }

    /**
     * Получает права на рокировку из FEN
     */
    private static int getCastlingRights(Board board) {
        int rights = 0;
        try {
            String fen = board.getFen();
            ...
            if (parts.length >= 3) {
                ...
                if (castlingPart.contains("K")) rights |= 1;
                if (castlingPart.contains("Q")) rights |= 2;
                if (castlingPart.contains("k")) rights |= 4;
                if (castlingPart.contains("q")) rights |= 8;
            }
        } catch (Exception e) {
            log.trace("Error parsing castling rights: {}", e.getMessage());
        }
        return rights;
    }
}